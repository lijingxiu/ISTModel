  ...
  if (xmlNode_ != NULL) {
    ...
    for (GDataXMLNode *child in children) {
      ...
      if (currNode->type == XML_ELEMENT_NODE) {
		...
		//Raik modify by 2012.12.27attributeForName
...
//MARK:: 直接取出节点下的所有属性然后去匹配 modify by liqiang
- (GDataXMLNode *)attributeForName:(NSString *)name {
  if (xmlNode_ != NULL) {
      NSArray *attributes = [self attributes];
      for (GDataXMLNode *attr in attributes) {
          NSString *theName = [NSString stringWithCString:[attr XMLNode]->name encoding:NSUTF8StringEncoding];
          if ([[name uppercaseString] isEqualToString:[theName uppercaseString]]) {
              return attr;
          }
      }
